...
`My Friends`区域现在也有自己的筛选功能：

```text
Filter: [__________] [Name ▼] [Search] [Reset]
```

这个筛选只会筛选当前用户自己的好友，不会筛选整个社交网络中的所有用户。下拉框支持：
...
```text
currentUser 的 friends
```

右边筛选的是：

```text
Network 里面的 all users
```
...
程序会从当前用户“朋友的朋友”里面寻找推荐对象。如果某个人还不是当前用户的好友，并且和当前用户有相同家乡或者相同工作地点，就会被放进推荐列表。
...
其中`Common Friends with Current User`表示当前登录用户和这个被查看用户的共同好友。如果打开的是当前用户自己，则不会显示共同好友区域。
...
目前程序已经可以完成基础的加载、登录、显示、增删用户、增删好友、搜索、保存、退出、当前用户好友筛选、共同好友和好友推荐。
...
1. 更完整的测试记录
2. 保存和重新加载后的验证
3. README / report / presentation 中对数据结构和功能的总结
4. 根据演示效果继续微调UI
```

### 当前测试情况

目前已经重新运行了`NetworkTester`，用来检查底层`Network`和文件读写相关逻辑是否仍然正常。
...
这说明目前底层的用户查询、登录检查、好友关系、筛选、朋友的朋友筛选、文件保存和文件读取没有明显问题。

不过还需要注意：这个tester主要测试的是Model层和文件管理逻辑，不是Swing UI的自动化测试。UI部分目前主要通过手动运行程序进行验证。
